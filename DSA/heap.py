...
• Parent-child relationships are maintained through index calculations:
  • For element at index i (in zero based indexing):
    • Left child: 2i + 1
    • Right child: 2i + 2
    • Parent: (i-1)/2
• The root is always at index 0
• This creates an implicit binary tree structure without storing explicit pointers
...
A heap can also be implemented as an explicit binary tree with nodes containing:
• Data value
...
This module has array based implementation for MIN HEAP which is the most common
...
class Heap:
    ...
    def has_right_child(self, arr: list, index: int) -> tuple:
        '''
        Inputs: heap array and index of the element to find the right child for
        '''
        idx = (2*index) + 2
    ...
    def has_left_child(self, arr: list, index: int) -> tuple:
        '''
        Inputs: heap array and index of the element to find the left child for
        '''
        idx = (2*index) + 1
        return (True, idx) if idx < len(arr) else (False, None)
    ...
    def has_parent(self, arr: list, index: int) -> tuple:
        '''
        Inputs: heap array and index of the element to find the parent for
        '''
    ...
    def heapify(self, arr: list) -> list:
        '''
        AI Description:
        Create a heap from an unsorted array by going bottom up and pushing the elements down.
        ...
        # The start_index + 1 is needed because range() is exclusive of the end value.
    ...
    def sift_down(self, arr: list, index: int, heap_size: int):
        ...
        while True:
            ...
            # if current is already smaller than the min child, heap property satisfied
            if arr[current] < arr[min_child_index]:
                break
            # swap and continue from the swapped position
            arr[current], arr[min_child_index] = arr[min_child_index], arr[current]
    ...
    def insert(self, val):
        ...
        Add the element at the end of array and sift up to reach the right spot
        '''
    ...
    def sift_up(self, arr, index):
        '''
        takes the index of the element from the heap and shifts it up
        until it reaches its right place
        '''
        while True:
    ...
    def remove(self, index):
        ...
        1. Replace the element to be deleted with the last element in the heap, then remove the last
           position.
        After replacement, the heap property might be violated in two possible directions:
        • **Upward violation**: New element is smaller than its parent (min-heap) or larger than parent (max-heap)
        • **Downward violation**: New element is larger than children (min-heap) or smaller than children (max-
        heap)
        2. If the new element is "better" than its parent (smaller in min-heap, larger in max-heap),
        bubble it up by repeatedly swapping with parent until heap property is restored.
        3. If the new element is "worse" than its children (larger in min-heap, smaller in max-heap),
        bubble it down by repeatedly swapping with the better child until heap property is restored.

        After replacement, you only need to fix in ONE direction - either up OR down, never both.
        Edge case handling:
        ...
        if index < 0 or index >= len(self.min_heap):
            ...
            return
        elif index == len(self.min_heap) - 1:  # deleting last leaf or last item of the array
            self.min_heap.pop()
            return
        # Replace last element with the element at index, then remove last element
        self.min_heap[index] = self.min_heap[-1]
        self.min_heap.pop()

        # Now we need to restore heap property at the new position
        left_present, left_index = self.has_left_child(self.min_heap, index)
        right_present, right_index = self.has_right_child(self.min_heap, index)
        parent_present, parent_index = self.has_parent(self.min_heap, index)

        if not left_present and not right_present:  # leaf with no children, always sift up
            self.sift_up(self.min_heap, index)
        else:
            # compare with parent and sift up if parent is worst than current index
            if parent_present and self.min_heap[index] < self.min_heap[parent_index]:
                self.sift_up(self.min_heap, index)
            else:
                ...
                if right_present:
                    children.append(right_index)
                if children:
                    min_child_index = min(children, key=lambda idx: self.min_heap[idx])
                    if self.min_heap[index] > self.min_heap[min_child_index]:
                        self.sift_down(self.min_heap, index, len(self.min_heap) - 1)

    def heap_sort(self) -> list:
        '''
        Heap Sort Algorithm (Descending Order with Min Heap):
        1. Build min heap from input array (already done in constructor)
        2. For i from (n-1) down to 1:
            a. Swap root (minimum) with element at position i
            b. Reduce heap size to i (exclude sorted portion)
            c. Sift down from root to restore heap property in reduced heap
        3. Result: Array sorted in descending order [largest...smallest]

        Key insight: Extract minimum to end positions, heap shrinks from right to left.
...
def tests():
    ...
    print(f"MIN HEAP array represenation after extracting max priority item is: {h.min_heap}")
    ins = 19
    h.insert(ins)
    print(f"MIN HEAP array represenation after inserting {ins} is: {h.min_heap}")
    print(f"HEAP IS EMPTY: {h.is_empty()}")
    rem = 2
    h.remove(rem)
    print(f"MIN HEAP array represenation after removing index {rem} is: {h.min_heap}")
